...
% % % 3) refractory period
% % % 4) beginning to end of 2nd stimulus
% % % 3) end of 2nd stimulus to end of simulation

simints = 5 ;           % increase intervals to 5 for 2nd spike 

Istim(1) = 0 ;          % define stimuli amplitudes
Istim(2) = stim_amp ;
Istim(3) = 0 ;
Istim(4) = stim_amp ;   % add for 2nd spike
Istim(5) = 0 ;          % add for 2nd spike

refracs = [6:21] ;      % set of refractory time intervals to test

figure                  % plot all results in a single window
for ii=1:length(refracs)
    
    stim_start = stimdelay ;
    stim_end = stimdelay + stimdur ;
    stim2_start = stim_end + refracs(ii) ;
    stim2_end = stim2_start + stimdur ;

    intervals(1,:) = [0,stim_start] ;
    intervals(2,:) = [stim_start,stim_end] ;
    intervals(3,:) = [stim_end,stim2_start] ;  % redefine and add intervals
    intervals(4,:) = [stim2_start,stim2_end] ; % for 2nd stimulus
...
% % Simulate 60 seconds at rest before stimulus applied
    ...
    hold on
    plot(t,V,colors(ii))
    title('Refractory Intervals')
    figurelegend{ii} = [num2str(refracs(ii)), ' ms']; 
